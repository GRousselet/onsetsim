...
erp = erp - min(erp);
erp = erp / max(erp);
temp2 = [zeros(1,79), erp, zeros(1,79)];
...
plot(Xf, noise(Nf, 1, srate, outvar), 'Color', [0 0.6 0], 'LineWidth',1)
plot(Xf, noise(Nf, 1, srate, outvar), 'Color', [0.6 0 0], 'LineWidth',1)
plot(Xf, noise(Nf, 1, srate, outvar), 'Color', [0 0 0.6], 'LineWidth',1)
plot(Xf, noise(Nf, 1, srate, outvar), 'Color', [1 0 0], 'LineWidth',1)
plot(Xf, noise(Nf, 1, srate, outvar), 'Color', [0 0 0], 'LineWidth',1)
...
disp(['Mean effect size = ', num2str(round(mean(es(125:127)), 2))])
...
for iter = 1:Nsim
    ...
    cohend(iter) = mean(allcohen(125:127));
    ...
    % find onset
    try
        onset_cluster(iter) = find_onset(sigcluster.elec_mask, Xf, 1);
    catch
    ...
    max_perm_th = prctile(maxt2_perm, pth*100); % MAX stat threshold
    try
        onset_max(iter) = find_onset(t2 > max_perm_th, Xf, 1);
    catch
        onset_max(iter) = NaN;
    end
    ...
    try
        onset_fdr(iter) = find_onset(pval_perm < pID, Xf, 1);
    catch
        onset_fdr(iter) = NaN;
    end
...
%% Simulation results: true positives, t^2, Nt=50, var=1
% Check results for one combination of variables.

Nsim = 10000;

true_onset = 160;
load([corepath,'/data/onsetsim_t2_n50_var1_eegnoise'])
...
%% ---------------------------------------------------------
%% Simulation: true positives, t^2, vary n, var=1
rng(666) % set random seed
Nsim = 10000;
Nperm = 2000;
...
for iter = 1:Nsim
    ...
    if iter == 1
        disp(['onsetsim_eeg tp t^2 vary n: ',num2str(iter),' / ',num2str(Nsim)])
    end
    if rem(iter,500) == 0
        disp(['onsetsim_eeg tp t^2 vary n: ',num2str(iter),' / ',num2str(Nsim)])
    end
    ...
    for T = 1:n_max
        cond1_all(:,T) = temp1 + noise(Nf, 1, srate, outvar);
        cond2_all(:,T) = temp2 + noise(Nf, 1, srate, outvar);
    end
    ...
    for iter_n = 1:n_length
        ...
        % find onset
        try
            onset_cluster(iter_n, iter) = find_onset(sigcluster.elec_mask, Xf, 1);
        catch
        ...
        max_perm_th = prctile(maxt2_perm, pth*100); % MAX stat threshold
        try
            onset_max(iter_n, iter) = find_onset(t2 > max_perm_th, Xf, 1);
        catch
        ...
        [pID,pN] = FDR(pval_perm, ath);
        try
            onset_fdr(iter_n, iter) = find_onset(pval_perm < pID, Xf, 1);
        catch
...
save([corepath,'/onsetsim_t2_nvar_var1_eegnoise'], ...
    'onset_cluster', 'onset_max', 'onset_fdr', 'onset_cp', ...
    'Nsim', 'Nperm', 'n_vec')

%% Simulation results: true positives, t^2, vary n, var=1, gamma=1

true_onset = 160;
load([corepath,'/onsetsim_t2_nvar_var1_eegnoise'])
Nn = length(n_vec);
...
figure('NumberTitle','off', 'Name', 'nt=50, var=1, vary n')

subplot(2,3,1) % Median onsets
hold on
...
subplot(2,3,2) % Median bias
hold on
plot(n_vec, bias(1,:), '-ko', 'LineWidth', 1, 'MarkerSize', 10, 'MarkerFaceColor', [0 0 0])
plot(n_vec, bias(2,:), '-ko', 'LineWidth', 1, 'MarkerSize', 10, 'MarkerFaceColor', [0.0980 0.3765 1.0000]);
plot(n_vec, bias(3,:), '-ko', 'LineWidth', 1, 'MarkerSize', 10, 'MarkerFaceColor', [0.6627 0.2941 0.0863]);
plot(n_vec, bias(4,:), '-ko', 'LineWidth', 1, 'MarkerSize', 10, 'MarkerFaceColor', [1 0 1]);
xticks(n_vec)
xlabel('Sample size')
ylabel('Bias')
ylim([0 100])
