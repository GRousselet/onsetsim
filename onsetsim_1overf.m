...
save([corepath,'/data/onsetsim_t2_n50_var1_gamma1'], ...
...
true_onset = 160;
load([corepath,'/data/onsetsim_t2_n50_var1_gamma1'])
...
for iter_gamma = 1:gamma_length
    ...
    rng(666) % set random seed here to make results directly comparable between gamma values
    ...
    for iter = 1:Nsim
        ...
        % save max Cohen'd
        allcohen = (mean(cond2, 2) - mean(cond1, 2)) ./ sqrt((var(cond1,0,2) + var(cond2,0,2)) / 2);
        cohend(iter_gamma, iter) = mean(allcohen(125:127));
        ...
        % find onset
        try
            onset_cluster(iter_gamma, iter) = find_onset(sigcluster.elec_mask, Xf, 1);
        catch
        ...
        max_perm_th = prctile(maxt2_perm, pth*100); % MAX stat threshold
        try
            onset_max(iter_gamma, iter) = find_onset(t2 > max_perm_th, Xf, 1);
        catch
        ...
        [pID,pN] = FDR(pval_perm, ath);
        try
            onset_fdr(iter_gamma, iter) = find_onset(pval_perm < pID, Xf, 1);
        catch
...
for iter = 1:Nsim
    ...
    for iter_n = 1:n_length
        ...
        % save max Cohen'd
        allcohen = (mean(cond2, 2) - mean(cond1, 2)) ./ sqrt((var(cond1,0,2) + var(cond2,0,2)) / 2);
        cohend(iter_n, iter) = mean(allcohen(125:127));
